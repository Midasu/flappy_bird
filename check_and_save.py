...
def main():
    ...
    sub_keys = []
    # Вытаскиваем все рабочие ключи из результатов
    for key, val in results.items():
        if isinstance(val, dict) and "top10" in val:
            for srv in val["top10"]:
                sub_keys.append(srv["key"])
        elif key == "other_countries":  # Обработка вложенной структуры
            for _, c_data in val.items():
                if isinstance(c_data, dict) and "top10" in c_data:
                    for srv in c_data["top10"]:
                        sub_keys.append(srv["key"])

    # Клиенты любят Base64-обертку, это спасает от ошибок кодировки
    sub_text = "\n".join(sub_keys)
    ...
    print("Подписка сохранена в docs/sub.txt")
